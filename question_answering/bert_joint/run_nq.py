...
def get_text_span(example, span):
    ...
    for i in range(span["start_token"], span["end_token"]):
        ...
        if not t["html_token"]:
            ...
            tokens.append(token)
    # print(TextSpan(token_positions, " ".join(tokens)))
    # input()
    return TextSpan(token_positions, " ".join(tokens))


def get_text_predicted(example, start, end):
    token_positions = []
    tokens = []
    for i in range(start, end):
        try:
            t = example.doc_tokens[i]
            token_positions.append(i)
            token = t.replace(" ", "")
            tokens.append(token)
        except:
            pass
            # print('QAS: %s \t Start: %s  End: %s ' %( example.qas_id, start, end))
            # print('Start: ', start)
            # print('End: ', end)
            # print('=========')
    return TextSpan(token_positions, " ".join(tokens))
...
def add_candidate_types_and_positions(e):
    ...
    for idx, c in candidates_iter(e):
        ...
        c["type_and_position"] = "[%s=%d]" % (
            context_type, counts[context_type])
...
class CreateTFExampleFn(object):
    ...
    def process(self, example):
        ...
        for input_feature in input_features:
            ...
            features["unique_ids"] = create_int_feature(
                [input_feature.unique_id])
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            answer_type_loss = compute_label_loss(
...
def compute_predictions(example, real):
    ...
    for c in example.candidates:
        ...
        if c["top_level"] and c["start_token"] <= start and c["end_token"] >= end:
            ...
            # print('===== CANDIDATE ====== ')
            # print(c["start_token"],  c["end_token"])
            break

    # print('========SPAN============')
    # print(long_span.start_token_idx)
    # print(long_span.end_token_idx)
    # print('=====================\n')

    # print(dir(example))
    # print('Candidates: ', example.candidates)
    # print('Results: ', example.results)
    # print(example.features)
    # print(get_candidate_text(example, example.candidates.))


    # print('===== INSIDE COMPUTE PREDICT==============')
    # # print(real.doc_tokens[long_span.start_token_idx:long_span.end_token_idx])
    # # print()
    # print()
    summary.predicted_label = {
        ...
        "long_answer": {
            ...
            "context": ' '.join(real.doc_tokens),
            "start_token": long_span.start_token_idx,
            "end_token": long_span.end_token_idx,
            "start_byte": -1,
            "end_byte": -1,
            "text": get_text_predicted(real, long_span.start_token_idx, long_span.end_token_idx).text
        ...
        "short_answers": [{
            ...
            "start_byte": -1,
            "end_byte": -1,
            "text": get_text_predicted(real, short_span.start_token_idx, short_span.end_token_idx).text
        }],
        "short_answers_score": score,
        "yes_no_answer": "NONE"
    }
    # print(summary.predicted_label['long_answer'])
...
def compute_pred_dict(candidates_dict, dev_features, raw_results, eval_examples):
    ...
    for f in dev_features:
        feature_ids.append(
            f.features.feature["unique_ids"].int64_list.value[0] + 1)
    ...
    examples = []
    # merged = [(i[0], i[1], j) for i, j in zip(features_by_id, , L3)]
    ...
    for e_pred, e_real in zip(examples, eval_examples):
        ...
        nq_pred_dict[e_pred.example_id] = summary.predicted_label
...
def validate_flags_or_throw(bert_config):
    ...
    if not FLAGS.do_train and not FLAGS.do_predict:
        raise ValueError(
            "At least one of `{do_train,do_predict}` must be True.")
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        nq_pred_dict = compute_pred_dict(candidates_dict, eval_features,[r._asdict() for r in all_results], eval_examples)
        
        predictions_json = {"predictions": list(nq_pred_dict.values())}
        with tf.gfile.Open(FLAGS.output_prediction_file, "w") as f:
            json.dump(predictions_json, f, indent=4)

        # # print(list(nq_pred_dict.values())[0]['long_answer']['text'])
        # # end = list(nq_pred_dict.values())[0]['long_answer']['end_token']
        # # print(list(nq_pred_dict.values())[0]['long_answer']['start_token'])
        # # print(list(nq_pred_dict.values())[0]['long_answer']['end_token'])
        # # print('Question: ', eval_examples[0].questions)
        # # print('Ans: ', eval_examples[0].answer)
        # # print('Start: ', eval_examples[0].start_position)
        # # print('End: ', eval_examples[0].end_position)
        # # print()
        # # print('Start: %d  End: %d' % (start,end))
        # # print(eval_examples[0].doc_tokens[start:end])
        # print('======== TOKENS =============')
        # # print(eval_examples[0].doc_tokens_map)
        # print('\n==========================\n')
        # texto = []
        # l = [260, 263, 264, 267, 268, 271, 272, 275, 276, 279, 280, 283, 284, 291, 292, 295, 296, 297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 313, 318, 319, 322, 323, 326, 327, 330, 331, 334, 335, 338, 339, 342, 343, 344]
        # for i in l:
        #     try:
        #         texto.append(eval_examples[0].doc_tokens[i])
        #     except:
        #         pass
        # print(' '.join(texto))
